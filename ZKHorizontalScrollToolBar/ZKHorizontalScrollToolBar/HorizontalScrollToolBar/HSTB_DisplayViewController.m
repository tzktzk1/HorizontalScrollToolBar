...
#import "HSTB_DisplayTitleLabel.h"
#import "HSTB_DisplayViewHeader.h"
#import "HSTB_FlowLayout.h"

static NSString * const ID = @"ContentCell";

@interface HSTB_DisplayViewController () <UICollectionViewDataSource,UICollectionViewDelegate>
{
    UIColor *_norColor;
    UIColor *_selColor;
}
...
@implementation HSTB_DisplayViewController
...
- (UIScrollView *)contentScrollView
{
    if (_contentScrollView == nil) {
        ...
        // 创建布局
        HSTB_FlowLayout *layout = [[HSTB_FlowLayout alloc] init];
...
- (void)viewDidLayoutSubviews
{
    ...
    if (_isInitial == NO) {
        ...
        if (_isFullScreen) {
            ...
            // 顶部标题View尺寸
            self.titleScrollView.frame = CGRectMake(0, titleY, HSTB_ScreenW, self.titleHeight);
        ...
        // 顶部标题View尺寸
        self.titleScrollView.frame = CGRectMake(0, 0, HSTB_ScreenW, self.titleHeight);
...
- (void)setUpAllTitle
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        UILabel *label = [[HSTB_DisplayTitleLabel alloc] init];
...
// 设置标题颜色渐变
- (void)setUpTitleColorGradientWithOffset:(CGFloat)offsetX rightLabel:(HSTB_DisplayTitleLabel *)rightLabel leftLabel:(HSTB_DisplayTitleLabel *)leftLabel
{
...
- (void)titleClick:(UITapGestureRecognizer *)tap
{
    ...
    if (vc.view) {
        ...
        // 发出重复点击标题通知
        if (_selIndex == i) {
            [[NSNotificationCenter defaultCenter] postNotificationName:HSTB_DisplayViewRepeatClickTitleNote object:vc];
        }
    }
...
- (void)selectLabel:(UILabel *)label
{
    ...
    for (HSTB_DisplayTitleLabel *labelView in self.titleLabels) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // 左边按钮
    HSTB_DisplayTitleLabel *leftLabel = self.titleLabels[leftIndex];
    
    // 右边角标
    NSInteger rightIndex = leftIndex + 1;
    
    // 右边按钮
    HSTB_DisplayTitleLabel *rightLabel = nil;
